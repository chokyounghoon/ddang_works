...
function SubsidiaryCard({
  ...
  return (
    <motion.div
      ...
      className={`
        relative rounded-2xl border p-5 backdrop-blur-sm overflow-hidden
        ${isHighlight
    ...
    >
      ...
      <div className="relative">
        <p className="text-2xl font-bold text-white tracking-tight">
          ...
          <span className="text-sm font-normal text-white/50 ml-1">억/년</span>
        </p>
        ...
        <p className="text-xs text-white/40 mt-1">100만 워커 기준 연간 추정</p>
      </div>
    </motion.div>
  );
}
      ...
      <div className="relative max-w-6xl mx-auto">
        ...
        <div className="text-center mb-12">
          <div className="inline-flex items-center gap-2 bg-emerald-500/10 border border-emerald-500/20 rounded-full px-4 py-1.5 mb-4">
            <LiveDot />
            <span className="text-xs text-emerald-400 font-medium">실시간 수익 집계 엔진</span>
          </div>
          ...
          <p className="text-white/50 max-w-xl mx-auto text-sm">
            트랜잭션 1건당 5개 계열사가 동시에 수익을 창출하는 Flywheel 구조.
            Cloudflare D1 원장에 실시간 적재됩니다.
          </p>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          {[
            ...
            {
              label: 'PG 수수료 절감 (vs 경쟁사)',
              value: data ? `₩${(Math.round(data.competitive.annualSavings / 100000000)).toLocaleString()}억/년` : '계산 중...',
              sub: `3% → 0% (100만 워커 기준)`,
            ...
            <motion.div
              ...
              className={`bg-gradient-to-br ${kpi.color} border ${kpi.border} rounded-2xl p-6 backdrop-blur-sm`}
            >
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6 backdrop-blur-sm">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-xs text-white/30">{data?.mode ?? 'MOCK'} MODE</span>
            </div>
            {data?.recentTransactions?.length ? (
              <TransactionFeed transactions={data.recentTransactions} />
            ) : (
              <div className="text-center text-white/30 py-8 text-sm">
                정산 트랜잭션 없음. 아래 버튼으로 시뮬레이션하세요.
              </div>
            )}
          </div>

          {/* 시뮬레이션 패널 */}
          <div className="bg-gradient-to-br from-indigo-500/10 to-purple-500/5 border border-indigo-500/20 rounded-2xl p-6 backdrop-blur-sm flex flex-col justify-between">
            <div>
              <h3 className="text-sm font-semibold text-white/80 mb-2">
                ⚡ 퇴근 정산 시뮬레이션
              </h3>
              <p className="text-xs text-white/50 mb-4">
                이지성 알바생 4시간 근무 완료 → BFF Saga 오케스트레이션 실행
              </p>
              <div className="space-y-2 text-xs">
                {[
                  { icon: '🏦', label: '신한은행', value: '₩49,850 에스크로 → 즉시 이체', color: 'text-blue-400' },
                  { icon: '☂️', label: '신한라이프', value: '보험료 ₩302 차감', color: 'text-emerald-400' },
                  { icon: '💳', label: '신한카드', value: 'ACS +5점 업데이트', color: 'text-red-400' },
                  { icon: '📈', label: '신한투자증권', value: '끝전 ₩850 → ETF 자동 매수', color: 'text-orange-400' },
                  { icon: '⚙️', label: '신한DS', value: 'BaaS 수수료 ₩200 과금', color: 'text-violet-400' },
                ].map(item => (
            ...
            <motion.button
              ...
              className={`
                ...
                ${simulating
                  ...
                  : 'bg-gradient-to-r from-indigo-500 to-purple-600 text-white hover:shadow-[0_0_30px_rgba(99,102,241,0.5)]'}
              `}
            >
              {simulating ? (
